...
# Visual Soft Assignment Coding--TODO:-- cuda out of memory
class VSACoding(nn.Module):
    ...
    def forward(self, imgs, caps, img_lens, cap_lens, return_attn=False):
        ...
        # att = torch.softmax(sims.masked_fill(mask == 0, -torch.inf),dim=-1) # Bi x Bt x K x L
        # att_imgs = torch.matmul(att,caps) # Bi x Bt x K x D
        # att_imgs = att_imgs/torch.norm(att_imgs,p=2,dim=-1,keepdim=True) 
        # sims = torch.mul(att_imgs.permute(1,0,2,3),imgs).permute(1,0,2,3).sum(dim=-1)
        # img_mask = get_mask(img_lens).permute(0,2,1).repeat(1,img_lens.size(0),1)
        # sims = sims.masked_fill(img_mask==0, -1)
